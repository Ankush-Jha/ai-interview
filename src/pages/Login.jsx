...
export default function Login() {
    ...
    return (
        <div className="min-h-screen flex bg-slate-50">
            {/* Left — Branding Panel */}
            <div className="hidden lg:flex lg:w-[45%] relative overflow-hidden bg-gradient-to-br from-primary-dark via-primary to-teal-400 flex-col justify-between p-12 text-white">
                {/* Decorative Shapes */}
                ...
                <div className="absolute top-1/2 right-1/4 w-32 h-32 bg-white/10 rounded-2xl rotate-45"></div>
                ...
                <div className="relative z-10 space-y-8">
                    <div>
                        <h1 className="text-4xl font-bold leading-tight mb-4">
                            Practice interviews<br />with AI that<br />
                            <span className="text-white/80">actually feels real</span>
                        </h1>
                        <p className="text-lg text-white/70 max-w-md leading-relaxed">
                            Conversational AI that adapts to your answers, asks follow-ups,
                            and gives you real-time feedback — just like a real interviewer.
                        </p>
                    </div>

                    {/* Feature Pills */}
                    <div className="flex flex-wrap gap-3">
                        {['Adaptive Follow-ups', 'Voice & Code Modes', '4 AI Personas', 'Instant Feedback'].map(f => (
                            <span key={f} className="px-4 py-2 bg-white/10 backdrop-blur-sm rounded-full text-sm font-medium border border-white/10">
                                {f}
                            </span>
                        ))}
                    </div>
                ...
                <div className="relative z-10">
                    <div className="flex items-center gap-3 pt-6 border-t border-white/10">
                        <div className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center text-sm font-bold">AJ</div>
                        <div>
                            <p className="text-sm font-medium">"The best way to prepare for interviews"</p>
                            <p className="text-xs text-white/60">10,000+ interviews conducted</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 flex items-center justify-center p-6 sm:p-12">
                <div className="w-full max-w-md space-y-8">
                    ...
                    <div className="text-center lg:text-left">
                        ...
                        <p className="mt-2 text-slate-500">Sign in to continue your interview prep</p>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <button onClick={handleGoogleLogin} disabled={loading} className="flex items-center justify-center gap-2 px-4 py-3 bg-white border border-slate-200 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-50 hover:border-slate-300 transition-all disabled:opacity-50">
                        ...
                        <button onClick={handleGithubLogin} disabled={loading} className="flex items-center justify-center gap-2 px-4 py-3 bg-white border border-slate-200 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-50 hover:border-slate-300 transition-all disabled:opacity-50">
